...
class BaseSignal(object):
   """
   A base class for Signals. Deriving classes must implement
   a calculateValueAt(self, x, x0, x1, y0, y1) method.
   """
   ...
   def getValueAt(self, x):
      ...
      for i in xrange(1, len(self._xs)):
         ...
         y1 = self._ys[i]
         if (x0 <= x < x1):
            answer = self.calculateValueAt(x, x0, x1, y0, y1)
            break
      return answer

   def calculateValueAt(self, x, x0, x1, y0, y1):
      raise NotImplementedError()

class StepSignal(BaseSignal):
   def __init__(self, xs, ys):
      BaseSignal.__init__(self, xs, ys)

   def calculateValueAt(self, x, x0, x1, y0, y1):
      return y0


class LinearInterpolatedSignal(BaseSignal):
   def __init__(self, xs, ys):
      BaseSignal.__init__(self, xs, ys)
...
def main():
   ...
   #proc.averageValue(BaseSignal(xs, ys), 0, 4, 100)
